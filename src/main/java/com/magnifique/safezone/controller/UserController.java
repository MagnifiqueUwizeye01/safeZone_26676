...
public class UserController {
    ...
    public Page<User> getUsersByLocationIdPaginated(
        ...
        Sort sort = Sort.by(sortDirection, sortBy);
        Pageable pageable = PageRequest.of(page, size, sort);
        return userService.getUsersByLocationId(locationId, pageable);
    }

    @GetMapping("/{userId}/province")
    public ResponseEntity<?> getProvinceByUserId(@PathVariable UUID userId) {
        Optional<Location> provinceOpt = userService.getProvinceByUserId(userId);
        if (provinceOpt.isPresent()) {
            return new ResponseEntity<>(provinceOpt.get(), HttpStatus.OK);
        }
        return new ResponseEntity<>("Province not found for this user", HttpStatus.NOT_FOUND);
    }
}
